...
from anti_confuser import anti_confuse
...
def unpack_mcpk(unpack_mode: str, file_path: str, output_dir: str) -> dict:
    ...
    with open(file_path, 'rb') as f:
        ...
        if dir_map[0]["files"].get(contents_json_hash):
            ...
            print(f"[+] 提取 contents.json (Directoty Hash: 00000000, File Hash: {contents_json_hash:08X})")
        ...
        if dir_map[0]["files"].get(redirect_mcs_hash):
            ...
            with open(os.path.join(output_dir, "redirect.mcs"), 'wb') as out_f:
                try:
                    ...
                    out_f.write(d_data)
                    success_count += 1
                    print(f"[+] 提取成功： redirect.mcs (目录哈希: 00000000, 文件哈希: {redirect_mcs_hash:08X})")
                    is_script_mcp = True
                except:
                    out_f.write(c_data)
                    error_count += 1
                    print(f"[+] 提取加密的 redirect.mcs (目录哈希: 00000000, 文件哈希: {redirect_mcs_hash:08X})")
            if unpack_mode != '3':
                if anti_confuse(unpack_mode,os.path.join(output_dir, "redirect.mcs")):
                    print(f"[+] redirect.mcs 已解密为redirect.py")
                else:
                    error_count += 1
                    success_count -= 1
                    print(f"[+] redirect.mcs 解密为py失败，跳过解密")
        ...
        if contents_data is not None:
            ...
            for file_item in files_to_extract:
                ...
                if d_hash not in dir_map:
                    print(f"[!] 没有找到目录哈希值： {norm_path}, 已跳过")
                    continue
                file_info = dir_map[d_hash]["files"].get(f_hash)
                if not file_info:
                    print(f"[!] 没有发现文件哈希值： {norm_path}, 已跳过")
                    continue
                ...
                try:
                    ...
                    with open(out_path, 'wb') as out_f:
                        out_f.write(u_data)
                    success_count += 1
                    print(f"[+] 提取成功1： {norm_path} (目录哈希={d_hash:08X}, 文件哈希={f_hash:08X})")
                    if unpack_mode != '3':
                        if anti_confuse(unpack_mode, out_path):
                            print(f"[+] {norm_path} 已解密为"+ os.path.splitext(norm_path)[0]+ ".py")
                        else:
                            error_count += 1
                            success_count -= 1
                            print(f"[+] {norm_path} 解密为py失败，跳过解密")
                except Exception as e:
                    error_count += 1
                    print(f"[!] 无法提取 {norm_path}, 保存已有数据 (目录哈希={d_hash:08X}, 文件哈希={f_hash:08X}): {e}")
                    out_path = os.path.join(output_dir, norm_path)
                    os.makedirs(os.path.dirname(out_path), exist_ok=True)
                    with open(out_path, 'wb') as out_f:
                        out_f.write(c_data)
                        ...
                        if head_magic == b'\x78\x9C' or head_magic == b'\x78\xDA':
                            ...
                            if file_name == '':
                                ...
                                with open(target_path, 'wb') as out_f:
                                    out_f.write(c_data)
                        success_count += 1
                        print(f"[+] 提取成功2： {name} (d_hash={d_hash:08X}, f_hash={f_hash:08X})")
                        if unpack_mode != '3':
                            if anti_confuse(unpack_mode, target_path):
                                print(f"[+] {name} 已解密为 " + os.path.splitext(name)[0] + ".py")
                            else:
                                error_count += 1
                                success_count -= 1
                                print(f"[+] {name} 解密为py失败，跳过解密")
                    except Exception as e:
                        error_count += 1
                        print(f"[!] 无法提取 {name}, 保存已有数据 (目录哈希={d_hash:08X}, 文件哈希={f_hash:08X}): {e}")
                        os.makedirs(out_dir, exist_ok=True)
                        with open(os.path.join(out_dir, name), 'wb') as out_f:
                            out_f.write(c_data)
                    f.seek(pos)
...
if __name__ == "__main__":
    ...
    print("[*] 与原版不同的是，该版本可以直接将所有文件解包至py文件，无需再对mcs使用解密")
    mcpk_path = input("[*] MCPK输入目录（必填）: ").strip('\"\'')
    output_directory = input("[*] 输出目录（不填默认在输入目录创建新文件夹）: ").strip('\"\'')
    print("[*] 选项：解包至py并删除多余文件(1)\n解包至py并保留mcs(2)\n仅解包至mcs(3)")
    unpack_mode = input('[*] 选项(1/2/3)：')
    if unpack_mode != '1' and unpack_mode != '2' and unpack_mode != '3':
        print("[!] 非法输入，程序退出")
        exit(1)
    if output_directory is None or output_directory.strip() == "":
        output_directory = (os.path.splitext(mcpk_path))[0] + '_unpacked'
    result = unpack_mcpk(unpack_mode,mcpk_path, output_directory)
    if result["is_success"]:
        print(f"[*] 解包完成，{result['success_count']}个mcs或py文件解析成功，{result['error_count']}个mcs或py文件解析失败")
        print("[*] 解包完成，输出目录："+ output_directory)
    else:
        print("[*] 解包失败")
